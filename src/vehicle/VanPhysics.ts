...
export class VanPhysics {
  ...
  /** Current speed in m/s (can be negative for reverse) */
  get speed(): number {
    return this._speed;
  }
  ...
  private distToNearestRoad(pos: number): number {
  ...
  private resolveCollision(
    ...
    if (!this.isOnRoad(newX, newZ)) {
      ...
      if (this.isOnRoad(prevX, newZ)) return { x: prevX, z: newZ };
      // Try sliding along X axis (revert only Z)
      if (this.isOnRoad(newX, prevZ)) return { x: newX, z: prevZ };
