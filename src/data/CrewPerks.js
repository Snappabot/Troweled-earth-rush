...
// ── Active Crew State ─────────────────────────────────────────────────────────
let _activeCrew = ['jose'];
export function setActiveCrew(ids) {
    _activeCrew = ['jose', ...ids.filter(id => id !== 'jose')];
}
export function getActiveCrew() { return _activeCrew; }
export function crewHas(id) { return _activeCrew.includes(id); }
export function getCrewMember(id) {
    return CREW_ROSTER.find(c => c.id === id);
}
/** Combined pay multiplier from all active crew */
export function crewPayMult() {
...
export function crewSpillGuard() {
...
export function crewTimerBonus() {
    return _activeCrew.reduce((acc, id) => {
        const c = getCrewMember(id);
