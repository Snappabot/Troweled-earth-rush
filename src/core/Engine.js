...
export class Engine {
    ...
    async init() {
        ...
        const ground = new THREE.Mesh(new THREE.PlaneGeometry(2048, 2048), new THREE.MeshLambertMaterial({ color: 0x4a7c4e }) // Green grass placeholder
        );
    ...
    createRoadGrid() {
        ...
        for (let x = -180; x <= 180; x += 40) {
            for (let z = -180; z <= 180; z += 40) {
                ...
                const building = new THREE.Mesh(new THREE.BoxGeometry(12 + Math.random() * 8, h, 12 + Math.random() * 8), buildingMat);
