 ...
 */
let _rawImg: HTMLImageElement | null = null;
...
export function isLogoReady(): boolean { return _rawImg !== null; }
...
function _drawFallbackTree(ctx: CanvasRenderingContext2D, size: number, shirtColor: number): void {
  const r = (shirtColor >> 16) & 0xFF;
  const g = (shirtColor >>  8) & 0xFF;
  const b =  shirtColor        & 0xFF;
...
function _drawFallbackTreeWhite(ctx: CanvasRenderingContext2D, size: number): void {
  ctx.strokeStyle = '#FFFFFF';
  ctx.fillStyle   = '#FFFFFF';
  _drawTreeStrokes(ctx, size);
}

function _drawTreeStrokes(ctx: CanvasRenderingContext2D, size: number): void {
  const s = size / 256;
