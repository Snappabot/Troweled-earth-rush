 ...
 */
import { MARBELLINO_COLOURS, PIGMENTS } from '../data/MarbellinoColours';
...
function computeMix(vals) {
    ...
    for (const col of MARBELLINO_COLOURS) {
        ...
        const dist = Math.sqrt((norm.B - cNorm.B) ** 2 +
            (norm.Y - cNorm.Y) ** 2 +
            (norm.L - cNorm.L) ** 2 +
            (norm.R - cNorm.R) ** 2 +
            (norm.G - cNorm.G) ** 2);
...
export class MarbellinoMixer {
    ...
    `;
      ...
      `;
            ...
            slider.type = 'range';
            slider.min = '0';
            slider.max = String(PIGMENT_MAX[key]);
            slider.step = String(PIGMENT_STEP[key]);
            slider.value = '0';
      ...
      `;
            ...
            this.valueEls[key + '_slider'] = slider;
        }
    ...
    _adjust(key, dir) {
        const step = PIGMENT_STEP[key];
        const max = PIGMENT_MAX[key];
        const precision = step < 1 ? 1 : 0;
        this.vals[key] = parseFloat(Math.max(0, Math.min(max, this.vals[key] + dir * step)).toFixed(precision));
        // Sync HTML slider
        const slider = this.valueEls[key + '_slider'];
        if (slider)
    ...
    _updatePreview() {
        ...
        // Update mix swatch
        if (this.mixCanvas)
            this._paintSwatch(this.mixCanvas, mixed);
        // Update match bar
        if (this.matchFill) {
    ...
    _triggerSuccess(pct) {
        ...
        if (this.submitBtn) {
            const formula = PIGMENTS
                .filter(p => this.target[p.key] > 0)
                .map(p => `${p.label}: ${this.target[p.key]}`)
                .join('  ·  ');
    ...
    // ── Styles ────────────────────────────────────────────────────────────────
    _injectStyles() {
        if (document.getElementById('mmixer-styles'))
            return;
