...
export class MarbellinoMixer {
    ...
    cannonTipX = 0;
    cannonTipY = 0;
    aimAngle = -0.78; // radians
    aimPower = 360;
    isAiming = false;
    hasFired = false; // has fired at least once this session
    // Active ball
    ball = null;
    // Hit effects
    splats = [];
    connieHitT = 0; // countdown for hit react
    bucketFlashT = 0;
    // Mix
    currentMix = '#F8F6F0';
    displayMix = '#F8F6F0';
    ...
    _fire() {
        ...
        if (this.shots[this.sel] >= pc.max) {
            if (this.fireBtn) {
                ...
                this.fireBtn.textContent = `MAX ${pc.max} shots!`;
    ...
    _triggerMix() {
        ...
        if (total === 0) {
            const h = document.getElementById('mmv3-hint');
            if (h) {
    ...
    _onMixDone() {
        ...
        if (pct >= 82) {
            ...
            this.mixState = 'idle';
            if (hint)
                hint.textContent = pct >= 65 ? `Getting close — try adjusting the shots` : `Not quite — clear and try again`;
            if (this.mixBtn) {
    ...
    _drawBucket(ctx, W, H) {
        ...
        // Handle
        ctx.beginPath();
        ctx.arc(cx, top - 3, topW / 2 - 5, Math.PI, 0);
    ...
    _drawConnie(ctx, W, H) {
        ...
        // Boots
        ctx.fillStyle = '#111';
        ctx.fillRect(cx - 10, cy - 12, 10, 12);
        ctx.fillRect(cx + 2, cy - 12, 10, 12);
        // Pants
        ctx.fillStyle = '#1a1a2a';
        ctx.fillRect(cx - 9, cy - bh * 0.44, 9, bh * 0.44 - 10);
        ctx.fillRect(cx + 2, cy - bh * 0.44, 9, bh * 0.44 - 10);
        // Shirt
        ctx.fillStyle = shirt;
        ctx.fillRect(cx - 12, cy - bh * 0.8, 24, bh * 0.37);
        // Neck + head
        ctx.fillStyle = skin;
        ...
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(cx - 8, cy - bh * 0.87);
        ctx.quadraticCurveTo(cx - 18, cy - bh * 0.92, cx - 14, cy - bh * 0.98);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx + 6, cy - bh * 0.87);
        ctx.quadraticCurveTo(cx + 16, cy - bh * 0.91, cx + 12, cy - bh * 0.96);
        ctx.stroke();
        ctx.lineCap = 'butt';
        // Connie's drill arm (toward bucket when mixing, relaxed otherwise)
        const drillX = this.bkX + 30;
        const drillY = this.bkTopY - 10;
        ...
        if (armActive) {
            ...
            ctx.beginPath();
            ctx.moveTo(cx - 10, cy - bh * 0.72);
            ctx.lineTo(cx - 16, cy - bh * 0.48);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(cx - 16, cy - bh * 0.47, 4.5, 0, Math.PI * 2);
            ctx.fillStyle = skin;
            ctx.fill();
        }
        // Right arm relaxed
    ...
    _drawJose(ctx, W, H) {
        ...
        ctx.fillStyle = '#888';
        ctx.beginPath();
        ctx.ellipse(canL, 0, 5, 9, 0, 0, Math.PI * 2);
        ctx.fill();
        // Barrel bands
        ctx.strokeStyle = '#555';
        ctx.lineWidth = 3;
        ...
        ctx.beginPath();
        ctx.roundRect(jx - 6, jy - 62, 16, 18, 3);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(jx + 4, jy - 50, 8, 0, Math.PI * 2);
        ctx.fillStyle = '#555';
        ctx.fill();
        ctx.strokeStyle = '#888';
        ctx.lineWidth = 1.5;
        ...
        for (let a = 0; a < Math.PI * 2; a += Math.PI / 3) {
            ctx.beginPath();
            ctx.moveTo(jx + 4, jy - 50);
            ctx.lineTo(jx + 4 + Math.cos(a) * 7, jy - 50 + Math.sin(a) * 7);
            ctx.stroke();
        }
        ...
        ctx.fillRect(jx - 10, jy - 12, 10, 12);
        ctx.fillRect(jx + 2, jy - 12, 10, 12);
        // Pants
        ctx.fillStyle = '#1a1a2a';
        ctx.fillRect(jx - 9, jy - bh * 0.44, 9, bh * 0.44 - 10);
        ctx.fillRect(jx + 2, jy - bh * 0.44, 9, bh * 0.44 - 10);
        // Shirt
        ctx.fillStyle = shirt;
        ctx.fillRect(jx - 12, jy - bh * 0.80, 24, bh * 0.37);
        // Neck
        ctx.fillStyle = skin;
        ctx.fillRect(jx - 4, jy - bh * 0.84, 8, bh * 0.055);
        // ── Head & Dreadlocks ──────────────────────────────────────────────────
        ...
        dreads.forEach((d, i) => {
            ...
            ctx.bezierCurveTo(jx + d.dx + d.swing * 0.5, headY + d.len * 0.35, jx + d.dx + d.swing, headY + d.len * 0.7, jx + d.dx + d.swing * 0.8 + Math.sin(i) * 2, headY + d.len);
            ctx.stroke();
        });
        ctx.lineCap = 'butt';
        ...
        ctx.beginPath();
        ctx.arc(jx - 3, headY - 2, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(jx + 3, headY - 2, 2, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(jx, headY + 2, 4, 0.2, Math.PI - 0.2);
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.moveTo(jx + 10, jy - bh * 0.72);
        ctx.quadraticCurveTo(jx + 16, jy - bh * 0.58, jx + 10, jy - 52 + 4);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(jx + 10, jy - 49, 4.5, 0, Math.PI * 2);
        ctx.fillStyle = skin;
        ctx.fill();
        // Left arm relaxed
        ctx.strokeStyle = shirt;
        ...
        ctx.beginPath();
        ctx.moveTo(jx - 10, jy - bh * 0.72);
        ctx.lineTo(jx - 16, jy - bh * 0.48);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(jx - 16, jy - bh * 0.47, 4.5, 0, Math.PI * 2);
        ctx.fillStyle = skin;
        ctx.fill();
        ctx.lineCap = 'butt';
        // Name tag
    ...
    _startTimer() {
        this.timerInt = setInterval(() => {
            ...
            if (this.timerEl) {
                ...
                if (this.timer <= 10)
                    this.timerEl.style.animation = 'mm3TimerPulse 0.5s ease infinite alternate';
            }
            if (this.timer <= 0)
    ...
    _resizeCanvas() {
        ...
        this.H = Math.round(240 * scale);
        this.canvas.width = this.W;
        this.canvas.height = this.H;
        // Scaled layout
        this.jX = Math.round(58 * scale);
        this.jY = Math.round(218 * scale);
        this.bkX = Math.round(258 * scale);
        this.bkTopY = Math.round(105 * scale);
        this.bkBotY = Math.round(168 * scale);
        this.cnX = Math.round(322 * scale);
        this.cnY = Math.round(218 * scale);
    }
    // ── Cleanup & styles ──────────────────────────────────────────────────────
    _cleanup() {
        this._stopTimer();
    ...
    _injectStyles() {
        if (document.getElementById('mmv3-styles'))
            return;
