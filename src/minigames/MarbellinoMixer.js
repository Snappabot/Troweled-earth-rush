...
function colDist(a, b) { const [ar, ag, ab] = hexRgb(a); const [br, bg, bb] = hexRgb(b); return Math.sqrt((ar - br) ** 2 + (ag - bg) ** 2 + (ab - bb) ** 2); }
function matchPct(a, b) { return Math.max(0, Math.round(100 - colDist(a, b) * 0.45)); }
function computeMix(shots) {
...
export class MarbellinoMixer {
    ...
    GRAVITY = 520;
    // ── Connie movement ──────────────────────────────────────────────────────
    connX = 265; // current Connie x in canvas px (scaled)
    connDir = -1; // +1 = right, -1 = left
    connSpeed = 0; // canvas px/s
    connPauseT = 0; // seconds paused at edge
    connMinX = 0; // clamped in _resizeCanvas
    connMaxX = 0;
    // Cannon
    cannonTipX = 0;
    cannonTipY = 0;
    ...
    _buildUI() {
      ...
      <div style="color:rgba(200,168,106,0.5);font-size:11px;margin-top:2px;">Hit the moving bucket — Connie won't hold still!</div>
        ...
        targetCol.appendChild(tsw);
        targetCol.appendChild(tlbl);
        ...
        this.canvas.addEventListener('pointerdown', (e) => {
            e.preventDefault();
            if (this.ball || this.mixState === 'mixing' || this.solved)
                return;
            this.isAiming = true;
            this._updateAim(e);
        }, { passive: false });
    ...
    `;
        ...
        brow.appendChild(undoBtn);
        brow.appendChild(this.fireBtn);
        brow.appendChild(this.mixBtn);
        brow.appendChild(twrap);
    ...
    _fire() {
        ...
        if (!this.hasFired && Math.abs(this.aimAngle + 0.78) < 0.05 && this.aimPower === 360) {
            if (this.fireBtn)
                this.fireBtn.textContent = '← Drag canvas to aim!';
            return;
        }
        const pc = PIGS.find(p => p.key === this.sel);
        if (this.shots[this.sel] >= pc.max) {
            if (this.fireBtn) {
                const o = this.fireBtn.textContent;
                this.fireBtn.textContent = `MAX ${pc.max}!`;
                setTimeout(() => { if (this.fireBtn)
                    this.fireBtn.textContent = o || '💥 FIRE'; }, 1200);
            }
            return;
        }
        const canL = 34;
        ...
        this.cannonTipX = tipX;
        this.cannonTipY = tipY;
        ...
        this.hasFired = true;
        if (this.fireBtn)
            this.fireBtn.textContent = '🔴 IN FLIGHT…';
    }
    _selectPig(key) {
        this.sel = key;
        const pc = PIGS.find(p => p.key === key);
        for (const p of PIGS) {
            const el = this.pigBtns[p.key];
            if (!el)
                continue;
            if (p.key === key) {
    ...
    _undoShot() {
        if (this.ball || this.mixState === 'mixing')
            return;
    ...
    _update(dt) {
        ...
        if (!this.solved && this.mixState !== 'success') {
            if (this.connPauseT > 0) {
                ...
                if (this.connX >= this.connMaxX) {
                    this.connX = this.connMaxX;
                    this.connDir = -1;
                    this.connPauseT = 0.25 + Math.random() * 0.6;
                }
                if (this.connX <= this.connMinX) {
                    this.connX = this.connMinX;
                    this.connDir = 1;
                    this.connPauseT = 0.25 + Math.random() * 0.6;
                }
        ...
        for (let i = this.splats.length - 1; i >= 0; i--) {
            this.splats[i].t += dt;
            if (this.splats[i].t > this.splats[i].maxT)
                this.splats.splice(i, 1);
        }
        if (this.connieHitT > 0)
            this.connieHitT -= dt;
        if (this.bucketFlashT > 0)
            this.bucketFlashT -= dt;
        // Mix animation
        ...
        if (this.mixState === 'success' || this.mixState === 'fail')
            this.mixT += dt;
        // Ball physics
        if (!this.ball)
            return;
    ...
    _hitConnie() {
        ...
        if (shout) {
            ...
            setTimeout(() => { if (shout) {
                shout.textContent = '';
                shout.style.color = 'rgba(200,168,106,0.4)';
            } }, 2000);
        }
        this.ball = null;
        this._resetFireBtn();
    ...
    // ── Mix ────────────────────────────────────────────────────────────────────
    _triggerMix() {
        if (this.mixState === 'mixing' || this.mixState === 'success' || this.solved)
            return;
        const total = Object.values(this.shots).reduce((a, b) => a + b, 0);
        if (total === 0) {
            const h = document.getElementById('mmv4-hint');
            if (h) {
                h.textContent = 'Get some shots in the bucket first!';
                setTimeout(() => { if (h)
                    h.textContent = ''; }, 1800);
            }
            return;
        ...
        if (this.mixBtn) {
            this.mixBtn.style.background = 'linear-gradient(135deg,#226622,#144414)';
            this.mixBtn.style.color = '#88FFcc';
            this.mixBtn.textContent = '⚙️ …';
        }
        if (this.fireBtn)
    ...
    _onMixDone() {
        ...
        if (bar) {
            bar.style.width = `${pct}%`;
            bar.style.background = pct >= 82 ? 'linear-gradient(90deg,#44DD88,#88FFCC)' : pct >= 55 ? 'linear-gradient(90deg,#C8A86A,#FFD080)' : 'linear-gradient(90deg,#882222,#CC4433)';
        }
        if (pctEl) {
            pctEl.textContent = `${pct}%`;
            pctEl.style.color = pct >= 82 ? '#44DD88' : pct >= 55 ? '#C8A86A' : '#FF6644';
        }
        if (pct >= 82) {
            ...
            this.mixState = 'idle';
            if (hint)
                hint.textContent = pct >= 65 ? 'Getting close — adjust the mix' : 'Way off — clear and start again';
            if (this.mixBtn) {
                this.mixBtn.style.background = 'linear-gradient(135deg,#8B4A00,#5A3000)';
                this.mixBtn.style.color = '#FFD080';
                this.mixBtn.textContent = '🔩 MIX';
            }
            if (this.fireBtn)
    ...
    _triggerSuccess(pct) {
        ...
        if (nameEl) {
            nameEl.textContent = this.target.name;
            nameEl.style.color = '#44DD88';
        }
        const hint = document.getElementById('mmv4-hint');
        if (hint) {
            const f = PIGS.filter(p => this.target.shots[p.key] > 0).map(p => `${p.key}=${this.target.shots[p.key]}`).join(' · ');
            hint.textContent = `✓ ${f}   (${getSolved().size}/${MIXER_FORMULAS.length} cracked)`;
            hint.style.color = '#44DD8888';
        }
        if (this.mixBtn) {
            this.mixBtn.textContent = '🏆 CRACKED!';
            this.mixBtn.style.background = 'linear-gradient(135deg,#2a8a44,#1a6030)';
            this.mixBtn.style.color = '#88FFcc';
    ...
    _triggerTimeout() {
        ...
        const nameEl = document.getElementById('mmv4-name');
        if (nameEl) {
            nameEl.textContent = `${this.target.name} ← answer`;
            nameEl.style.color = '#FF8866';
        }
        if (this.mixBtn) {
    ...
    _drawScene() {
        ...
        bg.addColorStop(0, '#1e1610');
        bg.addColorStop(1, '#090704');
        ctx.fillStyle = bg;
        ctx.fillRect(0, 0, W, H);
        ctx.strokeStyle = 'rgba(255,255,255,0.03)';
        ctx.lineWidth = 1;
        for (let yy = 20; yy < H; yy += 22) {
            ctx.beginPath();
            ctx.moveTo(0, yy);
            ctx.lineTo(W, yy);
            ctx.stroke();
        ...
        ctx.strokeStyle = 'rgba(200,168,106,0.2)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, H - 18);
        ctx.lineTo(W, H - 18);
        ...
        ctx.save();
        ctx.setLineDash([4, 6]);
        ctx.strokeStyle = 'rgba(200,100,30,0.15)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(this.connMinX, H - 14);
        ctx.lineTo(this.connMaxX, H - 14);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.restore();
        ...
        this._drawJose(ctx);
        if (this.isAiming && !this.ball)
            this._drawArcPreview(ctx);
        if (this.ball)
            this._drawBall(ctx);
    ...
    _drawBucket(ctx) {
        const cx = this.bkX;
        const top = this.bkTopY;
        const bot = this.bkBotY;
        const bkH = bot - top;
        const sc = this.W / 400;
        const topW = 54 * sc;
        const botW = 38 * sc;
        ...
        if (frac > 0) {
            ...
            ctx.beginPath();
            ctx.moveTo(cx - wAtFill / 2, fillTop);
            ctx.lineTo(cx + wAtFill / 2, fillTop);
            ctx.lineTo(cx + botW / 2, bot);
            ctx.lineTo(cx - botW / 2, bot);
            ctx.closePath();
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.ellipse(cx, top, topW / 2, 5.5 * sc, 0, 0, Math.PI * 2);
        ctx.strokeStyle = 'rgba(200,168,106,0.65)';
        ctx.lineWidth = 2;
        ctx.stroke();
        ctx.fillStyle = 'rgba(220,190,140,0.12)';
        ctx.fill();
        ctx.beginPath();
        ctx.arc(cx, top - 3 * sc, topW / 2 - 5 * sc, Math.PI, 0);
        ctx.strokeStyle = '#A08050';
        ctx.lineWidth = 3;
        ctx.stroke();
        ctx.font = `bold ${Math.round(10 * sc)}px system-ui`;
        ctx.fillStyle = 'rgba(200,168,106,0.5)';
        ctx.textAlign = 'center';
        ctx.fillText('TEM', cx, bot - bkH * 0.4);
        ctx.textAlign = 'start';
        ctx.restore();
    }
    _drawConnie(ctx) {
        ...
        ctx.fillStyle = shirt;
        ctx.fillRect(cx - 12 * (this.W / 400), cy - bh * 0.80, 24 * (this.W / 400), bh * 0.37);
        ctx.fillStyle = skin;
        ctx.fillRect(cx - 4 * (this.W / 400), cy - bh * 0.84, 8 * (this.W / 400), bh * 0.055);
        ctx.beginPath();
        ctx.arc(cx, cy - bh * 0.87, bh * 0.08, 0, Math.PI * 2);
        ctx.fill();
        // Wild blonde hair
        ctx.fillStyle = '#D4B840';
        ctx.beginPath();
        ctx.arc(cx, cy - bh * 0.87 - bh * 0.04, bh * 0.10, Math.PI, 0);
        ctx.fill();
        ctx.fillRect(cx - bh * 0.10, cy - bh * 0.87, bh * 0.20, bh * 0.13);
        ctx.strokeStyle = '#FFE070';
        ctx.lineWidth = 2.5 * (this.W / 400);
        ctx.lineCap = 'round';
        // Hair flies more when moving
        const hairSwing = (moving ? 1.8 : 1.0) * this.connDir;
        ctx.beginPath();
        ctx.moveTo(cx - 8 * (this.W / 400), cy - bh * 0.87);
        ctx.quadraticCurveTo(cx - 18 * (this.W / 400) + hairSwing * 4, cy - bh * 0.92, cx - 14 * (this.W / 400) + hairSwing * 6, cy - bh * 0.98);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx + 6 * (this.W / 400), cy - bh * 0.87);
        ctx.quadraticCurveTo(cx + 16 * (this.W / 400) + hairSwing * 4, cy - bh * 0.91, cx + 12 * (this.W / 400) + hairSwing * 6, cy - bh * 0.96);
        ctx.stroke();
        ctx.lineCap = 'butt';
        // Carry arm (toward bucket)
        const bkOffX = this.bkX - cx;
        const bkOffY = this.bkTopY - (cy - bh * 0.7);
        const armAngle = Math.atan2(bkOffY, bkOffX);
        if (mixing || success) {
            ctx.strokeStyle = shirt;
            ctx.lineWidth = 8 * (this.W / 400);
            ctx.lineCap = 'round';
            ctx.beginPath();
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.roundRect(-26 * (this.W / 400), -6 * (this.W / 400), 34 * (this.W / 400), 12 * (this.W / 400), 3);
            ctx.fill();
            ctx.stroke();
            ctx.save();
            ctx.translate(-30 * (this.W / 400), 0);
            ctx.rotate(spinAngle);
            const g = ctx.createRadialGradient(0, 0, 0, 0, 0, 11 * (this.W / 400));
            g.addColorStop(0, 'rgba(200,168,106,0.7)');
            g.addColorStop(1, 'transparent');
            ctx.fillStyle = g;
            ctx.beginPath();
            ctx.arc(0, 0, 11 * (this.W / 400), 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
            ctx.restore();
        }
            ...
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(this.bkX + 14 * (this.W / 400), this.bkTopY - 4 * (this.W / 400), 4.5 * (this.W / 400), 0, Math.PI * 2);
            ctx.fillStyle = skin;
            ctx.fill();
        }
        // Right arm relaxed
        ...
        ctx.beginPath();
        ctx.moveTo(cx + 10 * (this.W / 400), cy - bh * 0.72);
        ctx.lineTo(cx + 16 * (this.W / 400), cy - bh * 0.48);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(cx + 16 * (this.W / 400), cy - bh * 0.47, 4.5 * (this.W / 400), 0, Math.PI * 2);
        ctx.fillStyle = skin;
        ctx.fill();
        ctx.lineCap = 'butt';
        if (hit) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText('JOSE!!! 😤', cx, cy - bh - 6 * (this.W / 400));
            ctx.textAlign = 'start';
        }
        ...
        ctx.font = `bold ${Math.round(8 * (this.W / 400))}px system-ui`;
        ctx.fillStyle = 'rgba(255,200,80,0.65)';
        ctx.textAlign = 'center';
        ctx.fillText('CONNIE', cx, cy + 10 * (this.W / 400));
        ctx.textAlign = 'start';
        ctx.restore();
    }
    _drawJose(ctx) {
        ...
        // Cannon
        ctx.save();
        ctx.translate(jx, jy - 52 * sc);
        ctx.rotate(this.aimAngle);
        const brl = ctx.createLinearGradient(0, -9 * sc, 0, 9 * sc);
        brl.addColorStop(0, '#444');
        brl.addColorStop(0.5, '#777');
        brl.addColorStop(1, '#333');
        ctx.fillStyle = brl;
        ctx.beginPath();
        ctx.roundRect(0, -9 * sc, canL, 18 * sc, 4);
        ctx.fill();
        ctx.strokeStyle = '#222';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ...
        ctx.fillStyle = '#888';
        ctx.beginPath();
        ctx.ellipse(canL, 0, 5 * sc, 9 * sc, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#555';
        ctx.lineWidth = 3 * sc;
        [10, 20].forEach(bx => { ctx.beginPath(); ctx.moveTo(bx * sc, -9 * sc); ctx.lineTo(bx * sc, 9 * sc); ctx.stroke(); });
        ctx.restore();
        // Cannon mount
        ctx.fillStyle = '#3a2a1a';
        ctx.beginPath();
        ctx.roundRect(jx - 6 * sc, jy - 62 * sc, 16 * sc, 18 * sc, 3);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(jx + 4 * sc, jy - 50 * sc, 8 * sc, 0, Math.PI * 2);
        ctx.fillStyle = '#555';
        ctx.fill();
        ctx.strokeStyle = '#888';
        ctx.lineWidth = 1.5;
        ...
        for (let a = 0; a < Math.PI * 2; a += Math.PI / 3) {
            ctx.beginPath();
            ctx.moveTo(jx + 4 * sc, jy - 50 * sc);
            ctx.lineTo(jx + 4 * sc + Math.cos(a) * 7 * sc, jy - 50 * sc + Math.sin(a) * 7 * sc);
            ctx.stroke();
        }
        // Muzzle flash
        if (this.ball && Math.sqrt((this.ball.x - tipX) ** 2 + (this.ball.y - tipY) ** 2) < 40 * sc) {
            ...
            gf.addColorStop(0, 'rgba(255,200,50,0.9)');
            gf.addColorStop(1, 'transparent');
            ctx.fillStyle = gf;
            ctx.beginPath();
            ctx.arc(tipX, tipY, 18 * sc, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
        }
        // Body
        ctx.fillStyle = '#0a0a0a';
        ctx.fillRect(jx - 10 * sc, jy - 12 * sc, 10 * sc, 12 * sc);
        ctx.fillRect(jx + 2 * sc, jy - 12 * sc, 10 * sc, 12 * sc);
        ctx.fillStyle = '#1a1a2a';
        ctx.fillRect(jx - 9 * sc, jy - bh * 0.44, 9 * sc, bh * 0.44 - 10 * sc);
        ctx.fillRect(jx + 2 * sc, jy - bh * 0.44, 9 * sc, bh * 0.44 - 10 * sc);
        ctx.fillStyle = shirt;
        ctx.fillRect(jx - 12 * sc, jy - bh * 0.80, 24 * sc, bh * 0.37);
        ctx.fillStyle = skin;
        ctx.fillRect(jx - 4 * sc, jy - bh * 0.84, 8 * sc, bh * 0.055);
        ...
        dreads.forEach((d, i) => {
            ...
            ctx.lineWidth = 5 * sc;
            ctx.beginPath();
            ctx.moveTo(jx + d.dx * sc, headY - headR * 0.3);
        ...
        ctx.fillStyle = skin;
        ctx.beginPath();
        ctx.arc(jx, headY, headR, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#3a1a00';
        ctx.beginPath();
        ctx.arc(jx - 3 * sc, headY - 2 * sc, 2 * sc, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(jx + 3 * sc, headY - 2 * sc, 2 * sc, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(jx, headY + 2 * sc, 4 * sc, 0.2, Math.PI - 0.2);
        ctx.stroke();
        ctx.strokeStyle = '#8B4A00';
        ctx.lineWidth = 3.5 * sc;
        ctx.beginPath();
        ctx.arc(jx, headY, headR * 1.05, Math.PI * 0.75, Math.PI * 1.85);
        ctx.stroke();
        // Arms
        ctx.strokeStyle = shirt;
        ctx.lineWidth = 8 * sc;
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(jx + 10 * sc, jy - bh * 0.72);
        ctx.quadraticCurveTo(jx + 16 * sc, jy - bh * 0.58, jx + 10 * sc, jy - 52 * sc + 4 * sc);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(jx + 10 * sc, jy - 49 * sc, 4.5 * sc, 0, Math.PI * 2);
        ctx.fillStyle = skin;
        ctx.fill();
        ctx.strokeStyle = shirt;
        ctx.lineWidth = 8 * sc;
        ctx.beginPath();
        ctx.moveTo(jx - 10 * sc, jy - bh * 0.72);
        ctx.lineTo(jx - 16 * sc, jy - bh * 0.48);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(jx - 16 * sc, jy - bh * 0.47, 4.5 * sc, 0, Math.PI * 2);
        ctx.fillStyle = skin;
        ctx.fill();
        ctx.lineCap = 'butt';
        ctx.font = `bold ${Math.round(8 * sc)}px system-ui`;
        ctx.fillStyle = 'rgba(255,220,100,0.65)';
        ctx.textAlign = 'center';
        ctx.fillText('JOSE', jx, jy + 10 * sc);
        ctx.textAlign = 'start';
    }
    _drawArcPreview(ctx) {
        const pc = PIGS.find(p => p.key === this.sel);
        const vx = this.aimPower * Math.cos(this.aimAngle);
        const vy = this.aimPower * Math.sin(this.aimAngle);
        const sx = this.cannonTipX;
        const sy = this.cannonTipY;
        ctx.save();
        for (let i = 0; i < 28; i++) {
            const t = i * 0.055;
            const px = sx + vx * t;
            const py = sy + vy * t + 0.5 * this.GRAVITY * t * t;
            if (py > this.H || px > this.W)
                break;
            ctx.globalAlpha = Math.max(0, 0.7 - i * 0.022);
            ctx.fillStyle = pc.dot;
            ctx.shadowColor = pc.dot;
            ctx.shadowBlur = 6;
            ctx.beginPath();
            ctx.arc(px, py, 3 - i * 0.06, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.shadowBlur = 0;
        ctx.globalAlpha = 1;
    ...
    _drawSplats(ctx) {
        for (const s of this.splats) {
            ctx.save();
            ctx.globalAlpha = Math.max(0, (1 - s.t / s.maxT) * 0.8);
            ctx.fillStyle = s.color;
            ctx.beginPath();
            ctx.arc(s.x, s.y, 4 + s.t * 8, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
        }
    }
    ...
    _startTimer() {
        this.timerInt = setInterval(() => {
            ...
            if (this.timerEl) {
                ...
                if (this.timer <= 10)
                    this.timerEl.style.animation = 'mm4TimerPulse 0.5s ease infinite alternate';
            }
            if (this.timer <= 0)
                this._triggerTimeout();
        }, 1000);
    }
    _stopTimer() { if (this.timerInt) {
    ...
    _resizeCanvas() {
        ...
        this.H = Math.round(240 * sc);
        this.canvas.width = this.W;
        this.canvas.height = this.H;
        // Layout scaled
        this.jX = Math.round(58 * sc);
        this.jY = Math.round(218 * sc);
        this.bkTopY = Math.round(105 * sc);
        this.bkBotY = Math.round(168 * sc);
    ...
    _injectStyles() {
        if (document.getElementById('mmv4-styles'))
            return;
