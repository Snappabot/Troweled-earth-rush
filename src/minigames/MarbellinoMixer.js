...
function matchPct(mixed, target) {
...
export class MarbellinoMixer {
    ...
    // Animation state
    animState = 'idle';
    animT = 0; // time in current anim
    pumpColor = '#C8A020'; // color of currently pumped pigment
    pourDrops = [];
    currentMix = '#F8F6F0'; // computed mix hex
    displayMix = '#F8F6F0'; // smoothly lerped toward currentMix
    mixedResult = ''; // set after Connie mixes
    matchValue = 0; // set after mixing
    // UI elements
    dotEls = { B: [], Y: [], L: [], R: [], G: [] };
    timerEl = null;
    mixBtn = null;
    ...
    H = 200;
    bucketCX = 200;
    bucketY = 155; // bucket center bottom
    nozzleX = 150;
    nozzleY = 90; // where pour drops spawn
    joseX = 80;
    joseY = 178;
    connieX = 320;
    connieY = 178;
    ...
    // ── Shot system ───────────────────────────────────────────────────────────
    _pumpShot() {
        if (this.solved || this.animState === 'mixing' || this.animState === 'success')
            return;
    ...
    _removeShot() {
        if (this.solved || this.animState === 'mixing')
            return;
        const key = this.selectedPigment;
        if (this.shots[key] > 0) {
            this.shots[key]--;
    ...
    _onMixComplete() {
        ...
        if (pct >= 82) {
            ...
            if (hint)
                hint.textContent = pct >= 65 ? `Getting warmer… try adjusting the shots` : `Way off — reset and try again`;
            this.animState = 'idle';
        }
    }
    _triggerSuccess(pct) {
    ...
    _update(dt) {
        ...
        this.displayMix = rgbToHex(dr + (mr - dr) * spd * dt, dg + (mg - dg) * spd * dt, db + (mb - db) * spd * dt);
        // Update pour drops
        for (let i = this.pourDrops.length - 1; i >= 0; i--) {
            ...
            if (d.y > this.bucketY + 5 || d.alpha <= 0)
                this.pourDrops.splice(i, 1);
        }
        // State transitions
    ...
    _startTimer() {
        this.timerInt = setInterval(() => {
            ...
            if (this.timerEl) {
                ...
                if (this.timer <= 10)
                    this.timerEl.style.animation = 'mmTimerPulse 0.5s ease infinite alternate';
            }
            if (this.timer <= 0)
    ...
    // ── Styles ────────────────────────────────────────────────────────────────
    _injectStyles() {
        if (document.getElementById('mmv2-styles'))
            return;
