...
export class CoffeeShop {
  static readonly POS = { x: -60, z: -100 };
  static readonly TRIGGER_RADIUS = 12;
  private indicator: THREE.Mesh;
  private scene: THREE.Scene;
  private lastVisit = -30; // seconds — init to -30 so first visit works immediately
   ...
   */
  tryVisit(vanX: number, vanZ: number): boolean {
    if (!this.isNearby(vanX, vanZ)) return false;
    const now = Date.now() / 1000;
    if (now - this.lastVisit < 20) return false; // 20s cooldown
    this.lastVisit = now;
    return true;
  }
}
