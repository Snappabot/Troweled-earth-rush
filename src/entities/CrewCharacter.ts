...
export class CrewCharacter {
  ...
  private _makeLogo(cfg: CrewConfig): THREE.Texture {
    ...
    ctx.strokeStyle = fg; ctx.lineWidth = 6;
    ctx.beginPath(); ctx.moveTo(128, 198); ctx.lineTo(128, 108); ctx.stroke();
    for (const [x1,y1,x2,y2] of [[128,168,78,128],[128,168,178,128],[128,143,88,108],[128,143,168,108],[128,123,98,93],[128,123,158,93],[128,108,114,88],[128,108,142,88]] as [number,number,number,number][]) {
    ...
    ctx.font = 'bold 28px Arial'; ctx.textAlign = 'center'; ctx.fillText('TROWELED', 128, 230); ctx.fillText('EARTH', 128, 252);
