...
export class CrewCharacter {
    ...
    _build(cfg) {
        ...
        const addBox = (b, c, w, h, d, x = 0, y = 0, z = 0) => {
            const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat(c));
            m.position.set(x, y, z);
            m.castShadow = true;
            b.add(m);
        };
        const addCyl = (b, c, r, h, x = 0, y = 0, z = 0) => {
            const m = new THREE.Mesh(new THREE.CylinderGeometry(r, r, h, 8), mat(c));
            m.position.set(x, y, z);
            m.castShadow = true;
            b.add(m);
        };
        const addSphere = (b, c, r, x = 0, y = 0, z = 0) => {
        ...
        switch (cfg.hairStyle) {
            ...
            case 'dreadlocks':
                ...
                for (const [ox, oz] of [[-0.12, 0], [0.12, 0], [0, -0.16], [-0.08, -0.10], [0.08, -0.10]]) {
                    add(new THREE.CylinderGeometry(0.035, 0.022, 0.38, 6), ox, -0.07, oz);
                }
                break;
            case 'mohawk':
    ...
    _buildWalkClip() {
        ...
        const q = (rx, ry = 0, rz = 0) => Array.from(new THREE.Quaternion().setFromEuler(new THREE.Euler(rx * D, ry * D, rz * D)).toArray());
