...
const GRID = 40;
const ROAD_W = 8;
function buildPedestrian(bodyColor) {
...
export class PedestrianSystem {
    ...
    _spawn() {
        ...
        for (let i = 0; i < 20; i++)
            spawnPed('x'); // horizontal sidewalks (travel along X)
        for (let i = 0; i < 20; i++)
            spawnPed('z'); // vertical sidewalks (travel along Z)
    }
    _applyPosition(ped) {
        if (ped.axis === 'x') {
