        ...
        switch (sc.id) {
            case 'jose': {
                ...
                dreads.forEach(d => {
                    ...
                    ctx.bezierCurveTo(cx + d.dx + d.swing * 0.35, hy + d.len * 0.32, cx + d.dx + d.swing * 0.85, hy + d.len * 0.65, cx + d.dx + d.swing * 0.65, hy + d.len);
                    ctx.stroke();
                    // Knobby dread tip
                    ctx.beginPath();
                ...
                const jgw = hr * 0.95;
                const jgh = hr * 0.50;
                const jgy = hy - hr * 0.12;
                ctx.beginPath();
                ctx.roundRect(cx - jgw - 1, jgy - jgh / 2, jgw, jgh, 2);
                ctx.fill();
                ctx.stroke();
                ctx.beginPath();
                ctx.roundRect(cx + 1, jgy - jgh / 2, jgw, jgh, 2);
                ctx.fill();
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(cx - 1, jgy);
            ...
            case 'phil': {
                ...
                const pgw = hr * 0.95;
                const pgh = hr * 0.48;
                const pgy = hy - hr * 0.12;
                ctx.beginPath();
                ctx.roundRect(cx - pgw - 1, pgy - pgh / 2, pgw, pgh, 2);
                ctx.fill();
                ctx.stroke();
                ctx.beginPath();
                ctx.roundRect(cx + 1, pgy - pgh / 2, pgw, pgh, 2);
                ctx.fill();
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(cx - 1, pgy);
                ...
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(cx - pgw - 1, pgy);
                ctx.lineTo(cx - pgw - hr * 0.4, pgy - 1);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(cx + pgw + 1, pgy);
                ctx.lineTo(cx + pgw + hr * 0.4, pgy - 1);
