...
export class HUD {
    ...
    timerFailOverlay;
    crewPanelEl;
    moneyPanel; // exposed to GameMenu
    activeJob = null;
    activePhase = 1;
    flashTimeout = null;
    ...
    getMoneyPanel() { return this.moneyPanel; }
     ...
     */
    updateCrewStatus(crewToPickup, crewPickedUp, show) {
    ...
    showPenalty(jobTitle, penaltyAmt) {
        if (this.flashTimeout)
            clearTimeout(this.flashTimeout);
        this.flashOverlay.style.display = 'block';
        const flashMsg = this.flashOverlay.querySelector('div');
        if (flashMsg) {
