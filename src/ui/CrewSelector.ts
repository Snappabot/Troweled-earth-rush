...
export class CrewSelector {
  ...
  // ── Synergy hints ──────────────────────────────────────────────────────────
  private _updateSynergy(el: HTMLElement): void {
  ...
  private _makeAvatar(crew: CrewPerk, size: number): HTMLCanvasElement {
    ...
    switch (crew.id) {
      case 'jose': {
        ...
        ctx.beginPath(); ctx.roundRect(cx - gw - 0.5, gy - gh/2, gw, gh, 1.5); ctx.stroke();
        ctx.beginPath(); ctx.roundRect(cx + 0.5, gy - gh/2, gw, gh, 1.5); ctx.stroke();
        break;
      }
      case 'matt': {
      ...
      case 'fabio': {
        ...
        ctx.beginPath(); ctx.ellipse(cx, hy+hr*0.4, hr*0.55, hr*0.22, 0, 0, Math.PI); ctx.fill();
        break;
      }
      case 'phil': {
      ...
      case 'connie': {
        ...
        ctx.beginPath(); ctx.arc(cx, hy-hr*0.45, hr*1.08, Math.PI, 0); ctx.fill();
        ctx.fillRect(cx-hr*1.0, hy+hr*0.1, hr*2.0, size*0.16);
        break;
      }
      case 'mikayla': {
        ctx.fillStyle = '#8B4513';
        ctx.beginPath(); ctx.arc(cx, hy-hr*0.45, hr*1.05, Math.PI, 0); ctx.fill();
