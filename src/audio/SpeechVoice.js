...
const DEFAULT_VOICE = { lang: 'en-AU', pitch: 1.0, rate: 1.0, volume: 1.0 };
class SpeechVoiceEngine {
    ...
    _pickVoice(lang) {
        ...
        if (!v)
            v = voices.find(v => v.lang.startsWith(lang.split('-')[0])) ?? null;
        // Fallback to any English
        if (!v)
            v = voices.find(v => v.lang.startsWith('en')) ?? null;
